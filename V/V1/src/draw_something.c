        ...
        double ray_dir_y)
{
    ...
    if (distance < 1e-6)
        distance = 1e-6;
     // ———————————————————————————————————————————————————————
    // Projection en pixels écran :
    // distance est en pixels monde -> distance/BLOCK donne une distance en “cases”.
    // line_height = taille écran / distance_en_cases => plus loin = plus petit.
    // ———————————————————————————————————————————————————————
    line_height = (int)((double)HEIGHT / (distance / BLOCK));
     // On centre verticalement la tranche autour de la ligne d’horizon (milieu écran)
    ...
    if (draw_end >= HEIGHT)
        draw_end = HEIGHT - 1;
    // Petit ombrage : murs frappés via frontière horizontale plus sombres
    color = 0xCCCCCC;
    if (side == 1)
        color = 0x777777;
    // Dessin du mur (ici on ne gère ni ciel/sol ni textures
    y = draw_start;
    while (y <= draw_end)
    {
